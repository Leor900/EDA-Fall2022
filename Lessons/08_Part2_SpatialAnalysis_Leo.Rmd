...
upperNeuse_sf <- st_read("./Data/Spatial/NCHUC8.shp") %>% filter(SUBBASIN == "Upper Neuse")
...
wind_energy_huc8 <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12" = "factor")) %>% mutate(HUC_8 = substr(HUC_12,1,8)) %>% group_by(HUC_8) %>% summarize(kWhkm2day = sum(kWhkm2day))
  
#Join to HUC_8 features

huc8_sf_wind <- merge(x= huc8_sf, y= wind_energy_huc8, by.x = "HUC_8", by.y = "HUC_8")
...
In the exercise below, we'll subset one of the HUCs (Upper Neuese), and
...
the_triangle <- counties_sf_utm %>% filter(NAME %in% c("Chatham", "Durham", "Orange", "Wake"))
...
the_huc_clip <- huc2_utm %>% st_intersection(the_triangle)
