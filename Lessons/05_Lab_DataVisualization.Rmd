...
surface_temp <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), aes(x = daynum, y =temperature_C ))  + geom_point(aes(color=year4)) + facet_wrap(vars(lakename), nrow=2) + ylab("Surface Temperature") + xlab("Day of year")
...
temp_by_date <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C)) + geom_point(aes(color=depth), size= 0.5) + ylab("Temperature in Celsius") + scale_color_viridis(option = "magma", direction = -1) + scale_x_date(date_breaks = "5 years", date_labels = "%Y")

print(temp_by_date)

#limits = as.Date(c("1984-05-27", "2014-08-29"))
...
EPAair.Ex3 <- ggplot(EPAair, aes(x=PM2.5, y=Ozone)) + geom_point(aes(color=meanLat), alpha=0.5) + geom_smooth(method = lm, color="red") + ylab(expression("0"[3]* " AQI Value"))
