...
O3.boxplot <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) + geom_boxplot() + stat_summary(geom = "text", fun = max, vjust = -1, size = -4, label = c("b", "a")) + labs(x = "", y= "Ozone AQI value")
print(O3.boxplot)


O3.violin <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) + geom_violin(draw_quintiles = 0.5) + stat_summary(geom = "text", fun = max, vjust = -1, size = -4, label = c("b", "a")) + labs(x = "", y= "Ozone AQI value")
print(O3.violin)
...
PM2.5byOzone_regression <-  lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5byOzone_regression)
...
PM2.5byOzone_plot <- ggplot(EPAair, aes(x = Ozone, y = PM2.5)) + geom_point() + geom_smooth(method="lm", col = "blue") + geom_text(x = 100, y = 65, label = "PM2.5 = 15.63 + 0.38*Ozone", size = 3.5, color = "blue")

print(PM2.5byOzone_plot)
...
EPAair_subset <- 
  ...
  na.omit()
PM2.5_Corr <- cor(EPAair_subset)
corrplot(PM2.5_Corr, method = "ellipse")
corrplot.mixed(PM2.5_Corr, upper = "ellipse")
...
OzonebyAll.regression <- lm(data = EPAair_subset, PM2.5  ~ Ozone + Year + Month + meanLng + meanLat)
...
OzonebyBest.regression <- lm(data = EPAair_subset, PM2.5  ~ Ozone + Month + meanLng + meanLat)

summary(OzonebyBest.regression)
