...
ggplot(BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  labs(x = "Date", y = "Rainfall") + geom_smooth(method = lm)

ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  labs(x = "Date", y = "Temperature C") + geom_smooth(method = lm)
...
BRA_Temperature_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12) 
...
BRA_Rainfall_ts_decomposed <- stl(BRA_Rainfall_ts, s.window = "periodic")
plot(BRA_Rainfall_ts_decomposed)

BRA_Temperature_ts_decomposed <- stl(BRA_Temperature_ts, s.window = "periodic")
plot(BRA_Temperature_ts_decomposed)
